# -*- coding: utf-8 -*-
"""
Created on Sun Apr 24 21:02:56 2016
...
for y in raw_data:
    words = words + " " + re.sub("[^a-zA-Z\s]","",string.lower(y[1].translate(string.maketrans("",""), string.punctuation)))
...
for row in raw_data:
    raw_words = np.asarray(re.sub("[^a-zA-Z\s]","",string.lower(row[1].translate(string.maketrans("",""), string.punctuation))).split(" "))
...
for row in cluster_data:
    cursor.execute("INSERT INTO dbo.title_cluster_data_sneak VALUES (?, ?)", [str(row[0]),row[1]])
    cursor.commit()

for row in raw_mapping:
    cursor.execute("INSERT INTO dbo.title_job_words_sneak VALUES (?, ?)", [str(row[0]),row[1]])
    cursor.commit()
